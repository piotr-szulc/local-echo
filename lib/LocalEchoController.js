...
export default class LocalEchoController {
  ...
  applyPromptOffset(input, offset) {
    ...
    const newInputColorsRemoved = removeAnsiColors(newInput);

    return newInputColorsRemoved.length;
  }
  ...
  setInput(newInput, clearInput = true) {
    ...
    const newPromptNoAnsiColors = removeAnsiColors(newPrompt);
    ...
    const newLines = countLines(newPromptNoAnsiColors, this._termSize.cols);
    const { col, row } = offsetToColRow(
      newPromptNoAnsiColors,
      newCursor,
      this._termSize.cols
    );
